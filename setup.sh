...
installDepend() {
    ...
    if [ "$PACKAGER" = "pacman" ]; then
        DEPENDENCIES='arandr bat tree xarchiver multitail fastfetch wget unzip bspwm dconf dunst kitty nautilus thunar-volman thunar-archive-plugin nitrogen picom git pulseaudio-alsa pulseaudio-bluetooth ly yazi bluez bluez-utils brightnessctl htop npm python3-pip libconfig dbus libev meson pcre2 pixman uthash xcb-util-image xcb-util-renderutil cmake libxft libimlib2 libxinerama libxcb-res xorg-xev xorg-xbacklight alsa-utils rofi polybar sxhkd tldr'
    elif [ "$PACKAGER" = "nala" ] || [ "$PACKAGER" = "apt" ]; then
        DEPENDENCIES='bspwm sxhkd picom xarchiver kitty polybar rofi thunar thunar-archive-plugin thunar-volman nitrogen htop brightnessctl dunst git cmake meson npm python3 python3-pip dconf tldr fontconfig'
    elif [ "$PACKAGER" = "dnf" ]; then
        DEPENDENCIES='bspwm sxhkd picom xarchiver kitty polybar rofi thunar thunar-archive-plugin thunar-volman nitrogen htop brightnessctl dunst git gcc cmake meson npm python3 python3-pip dconf tldr fontconfig'
    else
